...
function Dashboard() {
  ...
  const { getProduct, order, user } = context
  return (
    <Layout>
      <div className=" button-container w-full flex justify-around mb-8 pb-12 mt-32 shadow-2xl">
        ...
        <button className="button text-2xl px-10 py-7 ">Users <span>({user.length})</span></button>
        <button className="button text-2xl px-10 py-7 ">Orders <span>({order.length})</span></button>
      </div>
      <DashTab/>
    </Layout>
  )
